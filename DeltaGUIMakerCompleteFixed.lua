...
local function InitDeltaGUIMaker()
    ...
    -- Variables (will be initialized properly in Start function)
    local Player = nil
    local Mouse = nil
    ...
    -- Enhanced RoundCorners function with error handling
    local function RoundCorners(instance, radius)
        -- Use pcall to handle potential errors
        local success, result = pcall(function()
            -- Check if the instance exists and is valid
            if not instance then
                return nil, "Instance is nil"
            end
            
            -- Check if instance already has a UICorner
            local existingCorner = instance:FindFirstChildOfClass("UICorner")
            if existingCorner then
                -- Update existing corner
                existingCorner.CornerRadius = UDim.new(0, radius or 8)
        ...
        -- Return the result or nil if there was an error
        if success then
            return result
        else
            -- Don't error, just warn and return nil
            warn("RoundCorners failed: " .. tostring(result))
            return nil
        end
    end
...
-- Create dropdown function with improved error handling
-- Enhanced mobile-friendly dropdown with comprehensive error handling
-- This function creates a touch-optimized dropdown control
local function CreateDropdown(parent, position, size, options, defaultOption, onChange)
    -- Constants for mobile optimization
    local DROPDOWN_HEIGHT = 40 -- Taller for better touch targets
    local OPTION_HEIGHT = 38   -- Taller for better touch targets
    local OPTION_PADDING = 8   -- More padding for better touch spacing
    local TEXT_SIZE = 16       -- Larger text for mobile readability
    local CORNER_RADIUS = 8    -- More rounded corners
    local MAX_DROPDOWN_HEIGHT = 250 -- Limit height for better mobile UX
    
    -- Input validation with detailed error messages
    if not parent then
        warn("CreateDropdown: Parent is nil")
        return nil
    end
    
    -- Validate options array
    local safeOptions = {"No options available"}
    if type(options) == "table" and #options > 0 then
        safeOptions = {}
        -- Filter out any nil values
        for _, option in ipairs(options) do
            if option ~= nil then
                table.insert(safeOptions, tostring(option))
            end
        end
        
        if #safeOptions == 0 then
            safeOptions = {"No options available"}
        end
    else
        warn("CreateDropdown: Invalid options provided, using default")
    end
    
    -- Safe defaultOption handling
    local safeDefaultOption = nil
    if defaultOption then
        -- Convert to string and check if it's in the options
        local defaultString = tostring(defaultOption)
        for _, option in ipairs(safeOptions) do
            if option == defaultString then
                safeDefaultOption = defaultString
                break
            end
        end
    end
    
    -- If default not found in options, use first option
    if not safeDefaultOption then
        safeDefaultOption = safeOptions[1]
    end
    
    -- Create dropdown container with error handling
    local dropdown, button, arrow, dropdownList, optionsContainer
    ...
    local success, error = pcall(function()
        ...
        dropdown.Size = size or UDim2.new(1, 0, 0, DROPDOWN_HEIGHT)
        dropdown.BackgroundColor3 = Color3.fromRGB(60, 150, 80)
        dropdown.BorderSizePixel = 0
        dropdown.ClipsDescendants = false -- Allow dropdown list to extend beyond container
        dropdown.Parent = parent
        
        -- Apply rounded corners safely
        RoundCorners(dropdown, CORNER_RADIUS)
        ...
        button.BackgroundTransparency = 0.1 -- Slight transparency for better visuals
        button.BorderSizePixel = 0
        button.Text = safeDefaultOption
        button.TextColor3 = Color3.fromRGB(255, 255, 255)
        button.Font = Enum.Font.GothamSemibold
        button.TextSize = TEXT_SIZE
        button.TextXAlignment = Enum.TextXAlignment.Left
        button.TextTruncate = Enum.TextTruncate.AtEnd -- Prevent text overflow
        ...
        RoundCorners(button, CORNER_RADIUS)
        
        -- Arrow indicator for dropdown state
        arrow = Instance.new("TextLabel")
        arrow.Name = "Arrow"
        arrow.Size = UDim2.new(0, 30, 0, 30) -- Larger for mobile
        arrow.Position = UDim2.new(1, -35, 0.5, -15)
        arrow.BackgroundTransparency = 1
        arrow.Text = "▼"
        arrow.TextColor3 = Color3.fromRGB(255, 255, 255)
        arrow.TextSize = TEXT_SIZE
        arrow.Font = Enum.Font.GothamBold
        arrow.ZIndex = 2 -- Ensure it's above the button
        arrow.Parent = button
        ...
        -- Calculate dropdown list height based on number of options (with limit)
        local calculatedHeight = math.min(#safeOptions * (OPTION_HEIGHT + OPTION_PADDING) + OPTION_PADDING, MAX_DROPDOWN_HEIGHT)
        dropdownList.Size = UDim2.new(1, 0, 0, calculatedHeight)
        dropdownList.Position = UDim2.new(0, 0, 1, 5)
        dropdownList.BackgroundColor3 = Color3.fromRGB(50, 130, 70)
        dropdownList.BorderSizePixel = 0
        dropdownList.Visible = false
        dropdownList.ZIndex = 100 -- High z-index to appear above other elements
        dropdownList.Parent = dropdown
        
        -- Make list shadow with glow effect for better visibility
        local shadow = Instance.new("UIStroke")
        shadow.Name = "Shadow"
        shadow.Thickness = 2
        shadow.Color = Color3.fromRGB(0, 0, 0)
        shadow.Transparency = 0.7
        shadow.Parent = dropdownList
        
        -- Apply rounded corners to dropdown list
        RoundCorners(dropdownList, CORNER_RADIUS)
        
        -- Options scrolling container for when there are many options
        ...
        optionsContainer.CanvasSize = UDim2.new(0, 0, 0, #safeOptions * (OPTION_HEIGHT + OPTION_PADDING))
        optionsContainer.AutomaticCanvasSize = Enum.AutomaticSize.Y
        optionsContainer.ScrollBarImageColor3 = Color3.fromRGB(255, 255, 255) -- White scrollbar
        optionsContainer.ScrollBarImageTransparency = 0.5 -- Semi-transparent
        optionsContainer.Parent = dropdownList
        
        -- Layout for option buttons
        local optionsList = Instance.new("UIListLayout")
        optionsList.SortOrder = Enum.SortOrder.LayoutOrder
        optionsList.Padding = UDim.new(0, OPTION_PADDING)
        optionsList.Parent = optionsContainer
        
        -- Add padding to list
        local listPadding = Instance.new("UIPadding")
        listPadding.PaddingTop = UDim.new(0, OPTION_PADDING/2)
        listPadding.PaddingBottom = UDim.new(0, OPTION_PADDING/2)
        listPadding.PaddingLeft = UDim.new(0, OPTION_PADDING/2)
        listPadding.PaddingRight = UDim.new(0, OPTION_PADDING/2)
        listPadding.Parent = optionsContainer
    end)
    
    if not success then
        warn("CreateDropdown: Failed to create dropdown core components: " .. tostring(error))
        return nil
    end
    
    -- Track selected option
    local selectedOption = safeDefaultOption
    local optionButtons = {}
    
    -- Create each option button with error handling
    for i, option in ipairs(safeOptions) do
        pcall(function()
            local optionButton = Instance.new("TextButton")
            optionButton.Name = "Option_" .. i
            optionButton.Size = UDim2.new(1, -OPTION_PADDING, 0, OPTION_HEIGHT)
            ...
            buttonPadding.Parent = optionButton
            
            -- Apply hover effect
            local normalColor = Color3.fromRGB(60, 150, 80)
            local hoverColor = Color3.fromRGB(70, 170, 90)
            local selectedColor = Color3.fromRGB(80, 190, 100)
            
            -- Update appearance based on selection state
            local function updateAppearance()
                if selectedOption == option then
                    optionButton.BackgroundColor3 = selectedColor
                    optionButton.TextSize = TEXT_SIZE + 1 -- Slightly larger text for selected option
                else
                    optionButton.BackgroundColor3 = normalColor
                    optionButton.TextSize = TEXT_SIZE
                end
            end
            ...
            optionButton.MouseButton1Click:Connect(function()
                ...
                for _, btn in pairs(optionButtons) do
                    if btn then pcall(updateAppearance) end
                end
    ...
    pcall(function()
        button.MouseButton1Click:Connect(function()
            ...
            if dropdownList.Visible and optionButtons[selectedOption] then
                optionsContainer.CanvasPosition = Vector2.new(0, 
                    (optionButtons[selectedOption].AbsolutePosition.Y - optionsContainer.AbsolutePosition.Y) - 
                    OPTION_HEIGHT/2)
            end
        end)
    end)
        ...
        if UserInputService then
            UserInputService.InputBegan:Connect(function(input)
                ...
                if (input.UserInputType == Enum.UserInputType.MouseButton1 or 
                   ...
                   dropdownList and dropdownList.Visible then
                    
                    local inputPosition = input.Position
                    if not inputPosition then return end
                    
                    -- Calculate bounds with error protection
                    local inDropdown, inList = false, false
                    ...
                    pcall(function()
                        ...
                        if dropdown and button then
                            ...
                            inDropdown = (inputPosition.X >= dropdownPos.X and 
                                          inputPosition.X <= dropdownPos.X + dropdownSize.X and
                                          inputPosition.Y >= dropdownPos.Y and 
                                          inputPosition.Y <= dropdownPos.Y + dropdownSize.Y)
                        end
                        ...
                        if dropdownList then
                            ...
                            inList = (inputPosition.X >= listPos.X and 
                                     inputPosition.X <= listPos.X + listSize.X and
                                     inputPosition.Y >= listPos.Y and 
                                     inputPosition.Y <= listPos.Y + listSize.Y)
                        end
                    end)
                    
                    -- Close if click is outside both dropdown and list
    ...
    -- Return the dropdown with value getter/setter methods
    return {
        Instance = dropdown,
        GetValue = function()
        ...
        SetValue = function(value)
            if value and optionButtons[value] then
                ...
                for _, btn in pairs(optionButtons) do
                    pcall(function() 
                        if btn.Text == selectedOption then
                            btn.BackgroundColor3 = Color3.fromRGB(80, 190, 100)
                            btn.TextSize = TEXT_SIZE + 1
                        else
        ...
        Refresh = function(newOptions, newDefault)
            ...
            -- Set new default
            if newDefault and type(newDefault) == "string" then
                for _, option in ipairs(safeOptions) do
                    if option == newDefault then
                        selectedOption = newDefault
                        button.Text = newDefault
                        break
                    end
                end
            else
                selectedOption = safeOptions[1]
                button.Text = safeOptions[1]
            end
            ...
            for i, option in ipairs(safeOptions) do
                pcall(function()
                    ...
                    if option == selectedOption then
                        optionButton.BackgroundColor3 = Color3.fromRGB(80, 190, 100)
                    ...
                    optionButton.MouseButton1Click:Connect(function()
                        ...
                        for _, child in pairs(optionsContainer:GetChildren()) do
                            if child:IsA("TextButton") then
                                if child.Text == selectedOption then
                                    child.BackgroundColor3 = Color3.fromRGB(80, 190, 100)
                                    child.TextSize = TEXT_SIZE + 1
                                else
                                    child.BackgroundColor3 = Color3.fromRGB(60, 150, 80)
                                    child.TextSize = TEXT_SIZE
                                end
                            end
                        end
                        ...
                        -- Call onChange callback if provided
                        if type(onChange) == "function" then
                            pcall(function() onChange(option) end)
                        end
                    end)
...
-- Enhanced show notification function with error handling
-- Enhanced ShowNotification function with comprehensive error handling and mobile optimization
function DeltaGUIMaker:ShowNotification(message, color)
    -- Define maximum concurrent notifications
    local MAX_NOTIFICATIONS = 3
    local NOTIFICATION_HEIGHT = 0.08
    local NOTIFICATION_GAP = 0.01
    
    -- Validate self and check for critical components
    if not self then
        warn("ShowNotification: Self reference is nil")
        return false
    end
    
    if not self.MainUI then
        warn("ShowNotification: Cannot show notification - MainUI is nil")
        return false
    end
    
    -- Validate and sanitize message
    local displayMessage = "Notification"
    if type(message) == "string" and message ~= "" then
        displayMessage = message
    elseif message ~= nil then
        -- Try to safely convert non-string message to string
        pcall(function()
            displayMessage = tostring(message)
        end)
    end
    
    -- Validate color parameter
    local displayColor = Color3.fromRGB(60, 60, 60) -- Default dark gray
    if typeof(color) == "Color3" then
        displayColor = color
    end
    
    -- Get existing notifications to position this one correctly
    local existingNotifications = {}
    for _, child in pairs(self.MainUI:GetChildren()) do
    ...
    if #existingNotifications >= MAX_NOTIFICATIONS then
        -- Remove oldest notification if at capacity
        local oldest = existingNotifications[1]
        if oldest and oldest.Parent then
            pcall(function() oldest:Destroy() end)
        end
        table.remove(existingNotifications, 1)
    end
    
    -- Create notification with extensive error handling
    local notification, text
    local notificationSuccess = false
    
    local creationSuccess, creationError = pcall(function()
        -- Create notification container
        notification = Instance.new("Frame")
        notification.Name = "Notification"
        notification.Size = UDim2.new(0.5, 0, NOTIFICATION_HEIGHT, 0)
        
        -- Position based on existing notifications
        local yPosition = 0.9 - (#existingNotifications * (NOTIFICATION_HEIGHT + NOTIFICATION_GAP))
        notification.Position = UDim2.new(0.25, 0, yPosition, 0)
        
        -- Styling
        notification.BackgroundColor3 = displayColor
        notification.BorderSizePixel = 0
        notification.ZIndex = 1000
        notification.BackgroundTransparency = 0.1 -- Start slightly transparent
        
        -- Create text container with proper text wrapping for mobile
        text = Instance.new("TextLabel")
        text.Size = UDim2.new(1, -20, 1, 0)  -- Padding on sides for better readability
        ...
        text.TextWrapped = true  -- Enable text wrapping for long messages
        text.ZIndex = 1001
        text.Parent = notification
        
        -- Add rounded corners with error handling
        pcall(function() 
            RoundCorners(notification, 10)
        end)
        
        -- Set parent after full configuration
        notification.Parent = self.MainUI
        
        -- Flag successful creation
        notificationSuccess = true
    end)
    
    -- Handle creation failure
    if not creationSuccess or not notificationSuccess then
        warn("ShowNotification: Failed to create notification - " .. tostring(creationError or "unknown error"))
        return false
    end
    
    -- Add slide-in and fade-out animations with error handling
    task.spawn(function()
        if not notification or not notification.Parent then return end
        
        -- Slide in animation (if TweenService is available)
        pcall(function()
            if TweenService then
                local startPosition = notification.Position
                notification.Position = UDim2.new(startPosition.X.Scale, startPosition.X.Offset, 
                                                 startPosition.Y.Scale + 0.1, startPosition.Y.Offset)
        ...
        local fadeTime = 3.0  -- Longer display time for mobile users
        
        -- Protected fade animation
        pcall(function()
            -- Delay destruction for proper visibility on mobile
            while notification and notification.Parent and tick() - fadeStart < fadeTime do
                -- Calculate transparency based on elapsed time
                local timeProgress = (tick() - fadeStart) / fadeTime
                local alpha = 1
                
                -- Only start fading in the second half of the display time
                if timeProgress > 0.5 then
                    alpha = 1 - ((timeProgress - 0.5) * 2)
                end
                ...
                -- Use RunService if available for smoother animation
                if RunService then
            ...
            if notification and notification.Parent then
                ...
                pcall(function()
                    if TweenService then
                        local endPosition = UDim2.new(notification.Position.X.Scale, notification.Position.X.Offset,
                                                      notification.Position.Y.Scale + 0.1, notification.Position.Y.Offset)
                        
                        local slideOutTween = TweenService:Create(notification,
                            TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
                            {Position = endPosition, BackgroundTransparency = 1})
...
-- Enhanced button tooltip for mobile with robust error handling
function DeltaGUIMaker:CreateButtonTooltip(button, tooltipText)
    -- Input validation
    if not button then
        warn("CreateButtonTooltip: Button is nil")
        return
    end
    ...
    -- Ensure self and MainUI exist
    if not self or not self.MainUI then
        warn("CreateButtonTooltip: Self or MainUI is nil")
        return
    end
    ...
    -- For mobile, show tooltip on longer touch (with error handling)
    local success, error = pcall(function()
        button.InputBegan:Connect(function(input)
            ...
            if input.UserInputType == Enum.UserInputType.Touch then
                ...
                if touchTimer then 
                    pcall(function() touchTimer:Disconnect() end)
                    touchTimer = nil
                end
                ...
                if RunService then
                    touchTimer = RunService.Heartbeat:Connect(function()
                        pcall(function()
                            if tick() - input.StartTime >= 0.7 and not tooltipShown then
                                -- Create tooltip with error handling
                                pcall(function()
                                    ...
                                    pcall(function() RoundCorners(tooltipInstance, 8) end)
                    ...
                    warn("CreateButtonTooltip: RunService is nil")
                end
            end
        end)
        
        -- Handle input end with error handling
        button.InputEnded:Connect(function(input)
            ...
            if input.UserInputType == Enum.UserInputType.Touch then
                ...
                if tooltipInstance then
                    pcall(function() tooltipInstance:Destroy() end)
                    tooltipInstance = nil
                end
    ...
    if not success then
        warn("CreateButtonTooltip: Failed to set up tooltip - " .. tostring(error))
    end
end
...
function DeltaGUIMaker:DeleteSelectedElement()
    if not self.SelectedElement then return end
    
    -- Save state for undo
    self:SaveState()
    
    -- Find and remove element from the list
    for i, elem in ipairs(self.Elements) do
        if elem.ID == self.SelectedElement.ID then
    ...
    if self.SelectedElement.Visual then
        self.SelectedElement.Visual:Destroy()
    end
...
function DeltaGUIMaker:Undo()
    if #self.UndoStack == 0 then return end
    
    -- Get last state and restore it
    local lastState = table.remove(self.UndoStack)
    
    -- Save current state for redo
    table.insert(self.RedoStack, DeepCopy(self.Elements))
    
    -- Restore elements from last state
    self.Elements = DeepCopy(lastState)
...
function DeltaGUIMaker:Redo()
    ...
    table.insert(self.UndoStack, DeepCopy(self.Elements))
    
    -- Restore elements from redo state
    self.Elements = DeepCopy(redoState)
...
-- Initialize and show
-- Enhanced Start function with comprehensive error handling
local function Start()
    -- Variables to track initialization status
    local initSuccess = false
    local maker = nil
    
    -- Safely wait for Player and services with timeout
    local function safeWaitForPlayer()
        -- First check if LocalPlayer is already available
        if Players and Players.LocalPlayer then
            return Players.LocalPlayer
        end
        
        -- If not, wait with a timeout
        local startTime = tick()
    ...
    local function checkServices()
        local requiredServices = {
            ...
            {name = "UserInputService", value = UserInputService},
            {name = "RunService", value = RunService},
            {name = "TweenService", value = TweenService},
            {name = "CoreGui", value = game:GetService("CoreGui")}
        }
        
        for _, service in ipairs(requiredServices) do
            if not service.value then
                return false, service.name .. " service is not available"
            end
        end
    ...
    -- Main initialization wrapped in pcall
    local success, result = pcall(function()
        -- Check required services
        local servicesOK, serviceError = checkServices()
        if not servicesOK then
            error(serviceError)
        end
        
        -- Wait for Player to be available
        Player = safeWaitForPlayer()
        if not Player then
            error("LocalPlayer not available after waiting. Try again later.")
        end
        
        -- Initialize Mouse with error handling
        local mouseSuccess, mouseError = pcall(function()
            Mouse = Player:GetMouse()
            if not Mouse then
                error("Mouse could not be initialized")
            end
        end)
        
        if not mouseSuccess then
            error("Mouse initialization failed: " .. tostring(mouseError))
        end
        
        -- Create and initialize the GUI maker with additional safeguards
        local createSuccess, createError = pcall(function()
            -- Create instance
            maker = DeltaGUIMaker.new()
            if not maker then
                error("DeltaGUIMaker instantiation returned nil")
            end
            
            -- Check critical properties
            if not maker.MainUI then
                error("MainUI is nil, initialization failed")
            end
            
            -- Set parent with safety check
            local coreGui = game:GetService("CoreGui")
            if not coreGui then
                error("CoreGui service is nil")
            end
            
            maker.MainUI.Parent = coreGui
            maker.IsOpen = true
            
            -- Version and author information (By MarKs)
            local authorLabel = Instance.new("TextLabel")
            authorLabel.Size = UDim2.new(0, 120, 0, 20)
            authorLabel.Position = UDim2.new(1, -125, 1, -25)
            authorLabel.BackgroundTransparency = 0.7
            authorLabel.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
            authorLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
            authorLabel.TextSize = 12
            authorLabel.Font = Enum.Font.Gotham
            authorLabel.Text = "By MarKs • v1.0.3"
            authorLabel.TextXAlignment = Enum.TextXAlignment.Center
            authorLabel.Parent = maker.MainUI
            
            -- Round corners on author label
            pcall(function() RoundCorners(authorLabel, 4) end)
            
            -- Try to show welcome notification
            pcall(function()
                maker:ShowNotification("Delta GUI Maker loaded!", Color3.fromRGB(80, 255, 120))
            end)
            
            -- Set global tracking flag for successful initialization
            initSuccess = true
        end)
        
        if not createSuccess then
            error("GUI Maker creation failed: " .. tostring(createError))
        end
        
        return maker
    end)
    
    -- Handle initialization errors with detailed feedback
    if not success or not initSuccess then
        warn("Delta GUI Maker failed to start: " .. tostring(result))
        
        -- Create protected error notification
        pcall(function()
            -- Create a simple error notification
            local errorGui = Instance.new("ScreenGui")
            errorGui.Name = "DeltaGUIMakerError"
            errorGui.ResetOnSpawn = false
            
            local errorFrame = Instance.new("Frame")
            errorFrame.Size = UDim2.new(0, 350, 0, 150)  -- Larger for more detailed error
            errorFrame.Position = UDim2.new(0.5, -175, 0.5, -75)
            errorFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
            errorFrame.BorderSizePixel = 0
            errorFrame.Parent = errorGui
            
            -- Try to round corners
            pcall(function()
                local uiCorner = Instance.new("UICorner")
            ...
            errorHeader.Size = UDim2.new(1, 0, 0, 30)
            errorHeader.Position = UDim2.new(0, 0, 0, 10)
            errorHeader.BackgroundTransparency = 1
            errorHeader.TextColor3 = Color3.fromRGB(255, 100, 100)
            errorHeader.TextSize = 18
            errorHeader.Font = Enum.Font.GothamBold
            errorHeader.Text = "Delta GUI Maker Error"
            errorHeader.Parent = errorFrame
            
            -- Error message with more details
            local errorText = Instance.new("TextLabel")
            errorText.Size = UDim2.new(1, -20, 1, -60)
            errorText.Position = UDim2.new(0, 10, 0, 40)
            errorText.BackgroundTransparency = 1
            errorText.TextColor3 = Color3.fromRGB(255, 255, 255)
            errorText.TextSize = 14
            errorText.Font = Enum.Font.GothamSemibold
            errorText.Text = "Failed to start: " .. tostring(result) .. 
                             "\n\nTry again later or refresh the page."
            errorText.TextWrapped = true
            errorText.TextYAlignment = Enum.TextYAlignment.Top
            errorText.Parent = errorFrame
            
            -- Try to set parent (may fail if CoreGui is not available)
            pcall(function()
                errorGui.Parent = game:GetService("CoreGui") or 
                                  game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
            end)
            
            -- Auto-close after 8 seconds (longer to read the detailed error)
            task.spawn(function()
                task.wait(8)
    ...
    -- Return the maker instance (might be nil if creation failed)
    return maker
end

    -- Start the GUI maker and store the instance locally
    local GUIMaker = Start()
    
    -- Return the main class and instance for external use
    return DeltaGUIMaker, GUIMaker
end

-- Execute the wrapped code and get the returns
local DeltaGUIModule, GUIMakerInstance = InitDeltaGUIMaker()

-- Export the module and active instance as globals for Delta executor
getgenv().DeltaGUIMaker = DeltaGUIModule
getgenv().GUIMaker = GUIMakerInstance

-- Return for module usage if needed
return DeltaGUIModule, GUIMakerInstance
